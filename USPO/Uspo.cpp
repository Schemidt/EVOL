			...
			else
			{
				...
				if (!timeReset)
				{
					...
					while (!std::regex_match(Selector, regex("[1-3]")))//повторяем ввод пока не будет цифра от 1 до 4
						Selector = _getch();//считываем буфер ввода
				...
				else
				{
					if (!vectload)
					{
						...
						while (!base.eof())
						{
							...
							sscanf_s(str.c_str(), "%lf %lf %lf %lf %lf %lf %lf %lf", &timeCol, &HighCol, &tangazCol, &VelocityCol, &StepCol, &Eng1Col, &Eng2Col, &RedCol);
